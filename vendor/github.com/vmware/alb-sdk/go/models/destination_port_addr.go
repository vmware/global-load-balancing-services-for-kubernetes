...
type DestinationPortAddr struct {
	...
	// TCP/UDP port range end (inclusive). Allowed values are 1-65535. Field introduced in 30.2.1. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
	DstPortEnd *uint32 `json:"dst_port_end,omitempty"`

	// TCP/UDP port range start (inclusive). Allowed values are 1-65535. Field introduced in 30.2.1. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
