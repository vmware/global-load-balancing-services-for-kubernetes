...
type GslbServiceDownResponse struct {
	...
	// Fallback IPV6 address to use in AAAA response to the client query when the GSLB service is DOWN. Field introduced in 18.2.8. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	FallbackIp6 *IPAddr `json:"fallback_ip6,omitempty"`

	// Fallback IP address to use in A response to the client having public IP when the GSLB service is DOWN or when no public pool is available. Field introduced in 31.2.1. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
	PublicFallbackIP *IPAddr `json:"public_fallback_ip,omitempty"`

	// Fallback IPV6 address to use in AAAA response to the client having public IP when the GSLB service is DOWN or when no public pool is available. Field introduced in 31.2.1. Allowed with any value in Enterprise, Enterprise with Cloud Services edition.
	PublicFallbackIp6 *IPAddr `json:"public_fallback_ip6,omitempty"`
