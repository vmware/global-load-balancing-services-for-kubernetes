...
type CCAgentProperties struct {
	...
	// Maximum polls to check for async jobs to finish. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	AsyncRetries *uint32 `json:"async_retries,omitempty"`

	// Delay between each async job status poll check. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	AsyncRetriesDelay *uint32 `json:"async_retries_delay,omitempty"`

	// Discovery poll target duration; a scale factor of 1+ is computed with the actual discovery (actual/target) and used to tweak slow and fast poll intervals. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	PollDurationTarget *uint32 `json:"poll_duration_target,omitempty"`

	// Fast poll interval. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
	...
	// Delay between each vnic status poll check. Unit is SEC. Allowed with any value in Enterprise, Essentials, Basic, Enterprise with Cloud Services edition.
