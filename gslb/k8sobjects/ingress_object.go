...
func parseVSAndControllerAnnotations(annotations map[string]string) (map[string]string, string, string, error) {
	...
	tenant, _ = annotations[gslbutils.TenantAnnotation]
...
func (ing IngressHostMeta) GetOldTenant() string {
	return ing.OldTenant
}
...
func (ing IngressHostMeta) GetIngressHostCksum() uint32 {
	...
	cksum += utils.Hash(ing.Cluster) + utils.Hash(ing.Namespace) +
		...
		utils.Hash(ing.VirtualServiceUUID) + utils.Hash(ing.ControllerUUID) + utils.Hash(ing.Tenant) + utils.Hash(ing.OldTenant)
	return cksum
}
