...
# Name of the vmware/global-load-balancing-services-for-kubernetes GIT branch for which the dev commits must be fetched
...
def _get_commits_since_time(credentials_tuple, 
    ...
    :param branch: name of the amko Git repository branch
...
def _setup_args():
    ...
    parser = argparse.ArgumentParser(
            description='Extracts all commits of the given amko branch since the specified timestamp or since the specified commit SHA')
    ...
    for p in [time_parser, commit_parser]:
        p.add_argument('-b',
                       ...
                       action='store',
                       type=str,
                       help='The name of the vmware/global-load-balancing-services-for-kubernetes.git GIT repository branch for which the developer commits must be fetched')
