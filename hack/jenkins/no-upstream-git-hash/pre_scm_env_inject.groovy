...
def get_essential_mapping(String branch_name, 
    ...
    // If upstream job is not defined, the same must be auto computed from amko_branch name
...
println("\n\nComputing and fetching head commit for amko\n")
amko_map = get_essential_mapping("${amko_branch}", 
                                "${amko_upstream_job}", 
...
map['AMKO_HEAD_COMMIT'] = amko_map['head_commit']
map['amko_upstream_job'] = amko_map['up_job']
