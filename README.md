...
2. Search the available charts for AMKO:
   ```
   $ helm show chart oci://projects.packages.broadcom.com/ako/helm-charts/amko --version 2.2.1

   Pulled: projects.packages.broadcom.com/ako/helm-charts/amko:2.2.1
...
3. Use the `values.yaml` from this repository to provide values related to Avi configuration. To get the values.yaml for a release, run the following command
   ...
   ```
   helm show values oci://projects.packages.broadcom.com/ako/helm-charts/amko --version 2.2.1 > values.yaml
...
5. Install AMKO:
   ```
   $ helm install --generate-name oci://projects.packages.broadcom.com/ako/helm-charts/amko --version 2.2.1 -f /path/to/values.yaml  --set configs.gsllbLeaderController=<leader_controller_ip> --namespace=avi-system
   ```
6. Check the installation:
   ...
   amko-1598451370	avi-system	1       	2023-10-14 11:16:21.889538175 +0000 UTC	deployed	amko-2.2.1	            2.2.1
   ```
...
```
helm template oci://projects.packages.broadcom.com/ako/helm-charts/amko --version 2.2.1 --include-crds --output-dir <output_dir>
```
...
NAME          	NAMESPACE 	REVISION	UPDATED                             	    STATUS  	CHART    	APP VERSION
amko-1598451370 avi-system	1       	2025-11-07 10:00:31.609195757 +0000 UTC	    deployed	amko-2.1.1 2.1.1
```

*Step4*

Get the values.yaml for the AMKO version 2.2.1 and edit the values as per the requirement.
...
```
helm upgrade amko-1598451370 oci://projects.packages.broadcom.com/ako/helm-charts/amko -f /path/to/values.yaml --version 2.2.1 --set configs.gslbLeaderController=<IP or Hostname> --set gslbLeaderCredentials.password=<username> --set gslbLeaderCredentials.username=<username> --namespace=avi-system
